...
|------|----------|------------|----------|------------|-------------------------|
| Linux | 主验证平台 | `./test.sh` | 历史文档化证据中 low-parallel configure/build 与 `persistence` 已记录 PASS；`T032` 当前最终扫尾里 configure/build/group-all/debug-tests 均为 FAIL | Linux Bash 主入口、`persistence` 主回归、Linux-specific durability / failure-injection / crash-style 解释边界 | 不声明当前 Linux 最终扫尾已全绿；不把 cluster/runtime-heavy 或当前入口失败解释成 Windows 已等价验证 |
...
- `T032` 当前最终扫尾结果：
  - `cmake --preset debug-ninja-low-parallel`：FAIL
  - `cmake --build --preset debug-ninja-low-parallel`：FAIL
  - `CTEST_PARALLEL_LEVEL=1 ./test.sh --group all --keep-data`：FAIL
  - `ctest --preset debug-tests --output-on-failure`：FAIL
- `ctest --preset debug-tests --output-on-failure` 当前未达到全绿
- 现存失败应归类为 cluster/runtime-heavy 运行期红灯
- `T032` 的 configure/build/group-all 失败也意味着当前不能把 Linux 最终验收
  扫尾写成“主入口与低并发构建已全绿”
